...
public class SimulationManager extends CloudSimEntity {
	...
	public static boolean issetlink(DataCenter device1, DataCenter device2) {	//几何可见建立链路
		...
		double p = (h1 + h2 + d) / 2.0;
		double areaSquared = p * (p - h1) * (p - d) * (p - h2);
		if (areaSquared <= 0 || Double.isNaN(areaSquared) || Double.isInfinite(areaSquared)) {
			return false;
		}
		double L = 2.0 * Math.sqrt(areaSquared) / d;
		return !Double.isNaN(L) && !Double.isInfinite(L)
				&& L > simulationParameters.MIN_HEIGHT + simulationParameters.EARTH_RADIUS;
	}
