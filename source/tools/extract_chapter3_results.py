...
def main() -> None:
    csv_files = list(Path("SatEdgeSim/output/chapter3").glob("*/Sequential_simulation.csv"))
    if not csv_files:
        csv_files = list(Path("SatEdgeSim/output").glob("*/Sequential_simulation.csv"))
    if not csv_files:
        raise FileNotFoundError("No Sequential_simulation.csv file found under SatEdgeSim/output")

    source = max(csv_files, key=lambda path: path.stat().st_mtime)
    output = source.with_name("chapter3_results.csv")
